...
Reaalses maailmas kasutatakse sageli **mõlemaid**. Lihtsustatud (aga kasulik) jaotus on selline: **Zabbix** katab hästi traditsioonilise IT‑infra (võrk, virtualisatsioon, failiserverid, UPS-id, printerid ja muu “legacy”), samas kui **Prometheus** sobib eriti hästi konteinerplatvormidele ja dünaamilisse keskkonda (service discovery, cloud‑native mustrid).
...
**Agent 1** (C, stabiilne klassika) ja **Agent 2** (Go, uuem ja pluginatega).
...
**Agent 1** on klassikaline ja väga levinud. **Agent 2** on uuem ning selle tugevus on **pluginad** (laiendatavus). Tootmises kohtad tihti mõlemat korraga, sõltuvalt hosti rollist ja ajaloost.

Praktiline rusikareegel:
“standard Linux host” puhul sobivad mõlemad, aga kui vajad lisa‑integratsioone või tahad tulevikukindlamat valikut, on Agent 2 sageli mõistlikum.

### 4.2 Kuidas “agentita” monitooring Zabbixis välja näeb

Zabbix ei nõua alati hosti peal agenti. Väga palju asju tuleb “otse”: **SNMP** (võrguseadmed, UPS-id, printerid), **HTTP Agent** (REST API-d/endpointid), **ICMP** (ping/latency), **IPMI** (riistvara), **JMX** (Java), **VMware** (vCenter/ESXi).
...
Oletame, et sul on 3000 item’it ja igaüks uueneb iga 60 sekundi järel. Siis on **NVPS = 3000 / 60 = 50 väärtust sekundis**.
...
??? tip "NVPS kontroll tootmises"
    ...
    vaata sisemisi item’eid, mis näitavad tegelikku kirjutusmahtu ja järjekordi. Kui NVPS on 2× suurem kui arvasid, on tihti põhjus “liiga tihe intervall” või “liiga palju item’eid template’is”.
...
| **Kirjutamise sünkroniseerimine** | konservatiivne flush on aeglane; Zabbix talub sageli “natuke leebemat” garantiid (kiirem kirjutus) |
...
Proxy groupid ei sobi igasse olukorda. Peamised piirangud on: **SNMP trapid** (ei toeta), **agentiversioonid** (aktiivses režiimis eeldab 7.0+), **tulemüüri reaalsus** (agent peab jõudma *kõigi* grupi proksideni), ning “päriselus” tuleb **välised skriptid** hoida prokside vahel sünkroonis. VMware puhul tasub olla ettevaatlik, sest vCenter’i päringukoormus võib suureneda.
...
Tüüpiline väärtus:
vähem “DB + upgrade + backup” tööd, kiirem stardikiirus (eriti väiksematel tiimidel) ja sobib siis, kui tahad Zabbixi funktsionaalsust, aga mitte Zabbixi platvormi ise opereerida.

Piirangud/kaalutlused:
võrguühendus Cloudi (agendid/proksid peavad jõudma), andmete asukoht/compliance ning kulumudel (subscription).
...
| **Modelleerimine** | hostid, grupid, template’id, discovery (LLD) |
| **Korrigeerimine** | preprocessing (regex/JSONPath/JS), dependent items |
| **Alerting** | triggerid, event correlation, actions, maintenance/silences |
| **Visualiseerimine** | graafikud, dashboardid, mapid/geomap |
| **Automatiseerimine** | API, import/export, webhook‑integratsioonid |
